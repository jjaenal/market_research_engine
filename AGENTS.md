...
- `requirements.txt` is intentionally empty; there is **no `pyproject.toml`, no test runner, no linter configured**. Nothing can be installed or tested yet. Do not invent commands like `pytest`/`ruff` and expect them to work.
