...
- `docs/00-foundation/FND-006_Project_Status.md` (Active) is authoritative for current phase/sprint/milestone/next steps. As of now: Foundation is 6/9 complete; next is FND-007 (Roadmap) → FND-008 (TODO) → FND-009 (Glossary), then Product Definition. Check it before deciding what to work on.
...
- `docs/00-foundation/FND-004_Document_Index.md` (Active) is the official registry of all doc IDs, paths, versions, and statuses. **Check it before creating a doc** (duplicate prevention); a doc not listed there is not official. Every new doc must be added to FND-004 (workflow: assign ID → create doc → update FND-004). Note: FND-004 is currently out of sync — FND-005 and FND-006 exist (Active) but are still listed as `Planned`; update the index when it lags (FND-004 §25).
