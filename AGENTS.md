...
- `docs/00-foundation/FND-006_Project_Status.md` (Active) is authoritative for current phase/sprint/milestone/next steps. As of now: Foundation is 8/9 complete; remaining is FND-009 (Glossary) plus a Foundation Review, then Product Definition. Check it before deciding what to work on.
...
- `docs/00-foundation/FND-008_TODO.md` (Active) is the execution backlog (statuses PLANNED/READY/IN_PROGRESS/BLOCKED/REVIEW/DONE/DEFERRED/CANCELLED, priorities P0–P3). Its current next action is FND-009; M1 Product Definition comes after Foundation Review.
...
- `docs/00-foundation/FND-004_Document_Index.md` (Active) is the official registry of all doc IDs, paths, versions, and statuses. **Check it before creating a doc** (duplicate prevention); a doc not listed there is not official. Every new doc must be added to FND-004 (workflow: assign ID → create doc → update FND-004). Note: FND-004 is currently out of sync — FND-005 through FND-008 exist (Active) but are still listed as `Planned`; update the index when it lags (FND-004 §25).
